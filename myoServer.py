...
#emgdc = dict()
#imgdc = dict()
emgdf = pd.DataFrame(columns= ['em1', 'em2', 'em3', 'em4', 'em15', 'em6', 'em7', 'em8'] )
#imgdf = pd.DataFrame(columns= ['accel_x', 'accel_y', 'accel_z', 'gyro_x', 'gyro_y', 'gyro_z', 'quat_x', 'quat_y', 'quat_z', 'quat_w', 'roll', 'pitch', 'yaw' ] )
...
def EMGHandler(addr, tags, data, client_address):
    ...
    txt += str(data)
    #print stime
    #emgdc[stime] = np.array(data)
    #print 'vtime', vtime, '************', np.array(data)
    emgdf.ix[stime] = data;
    print(txt)
...
if __name__ == "__main__":
    ...
    global count
    t = threading.Thread(name='ploting', target= plot_emg)
    t.start()
